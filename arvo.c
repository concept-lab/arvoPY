...
/*
Author: L.Gagliardi IIT
Contains implementation in C of ARVO (no credits, see below) to be used with Ctypes for python interface.
INSTRUCTIONS 
Create the library using:
    gcc -fPIC -c arvo.c -lm
OBS: might need : gcc -fPIC -c arvo.c -lm -std=c99
and then 
    gcc -shared arvo.o -o libArvo.so
Within python script:
import os
import ctypes
libfile = os.path.abspath("libArvo.so") 
 ...
 *
 * Version history:
 * V2.1 (L. Gagliardi IIT): spheres passed as an argument to main, and returns volume and surface. 
 * Bug correction lines 145, 156
